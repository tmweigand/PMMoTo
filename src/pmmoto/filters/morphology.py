...
def gen_struct_ratio(subdomain, radius):
    ...
    struct_ratio = np.array(
        [
            math.ceil(radius / subdomain.resolution[0]),
            math.ceil(radius / subdomain.resolution[0]),
            math.ceil(radius / subdomain.resolution[1]),
            math.ceil(radius / subdomain.resolution[1]),
            math.ceil(radius / subdomain.resolution[2]),
            math.ceil(radius / subdomain.resolution[2]),
        ],
        dtype=np.int64,
    )
...
def gen_struct_element(subdomain, radius):
    ...
    """
    voxel = subdomain.resolution
    struct_ratio = gen_struct_ratio(subdomain, radius)

    _x = np.linspace(
        -struct_ratio[0] * subdomain.resolution[0],
        struct_ratio[0] * subdomain.resolution[0],
        struct_ratio[0] * 2 + 1,
    )
    _y = np.linspace(
        -struct_ratio[1] * subdomain.resolution[1],
        struct_ratio[1] * subdomain.resolution[1],
        struct_ratio[1] * 2 + 1,
    )
    _z = np.linspace(
        -struct_ratio[2] * subdomain.resolution[2],
        struct_ratio[2] * subdomain.resolution[2],
        struct_ratio[2] * 2 + 1,
    )
...
def morph_add(subdomain, grid, radius, fft=False):
    ...
    if fft:
        ...
        _grid_distance = edt.edt3dsq(
            np.logical_not(halo_grid), anisotropy=subdomain.resolution
        )
...
def morph_subtract(subdomain, grid, radius, fft=False):
    ...
    if fft:
        ...
        _pad = 1
        if any(subdomain.boundaries == 0):
            _pad = np.max(struct_ratio)
        _grid = np.pad(
            array=halo_grid, pad_width=_pad, mode="constant", constant_values=1
        )
        ...
        _grid_distance = edt.edt3dsq(halo_grid, anisotropy=subdomain.resolution)
