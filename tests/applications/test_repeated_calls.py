...
def test_repeated_call():
    ...
    sphere_data = sphere_data * 1.1
    pm_bigger = pmmoto.domain_generation.gen_pm_spheres_domain(
        sd, sphere_data, domain_data
    )
    pm_bigger_sum = np.sum(pm_bigger.grid)

    print(f"RE-DO: Sum: {pm_bigger_sum} Domain Size {sd.domain.size_domain}")

    sd_bigger = pmmoto.initialize(
        rank, size, subdomains, nodes, boundaries, inlet, outlet
    )
    pm_new = pmmoto.domain_generation.gen_pm_spheres_domain(
        sd_bigger, sphere_data, domain_data
    )
    pm_new_sum = np.sum(pm_new.grid)

    print(
        f"Re-Initialize: Sum: {pm_new_sum} Domain Size {sd_bigger.domain.size_domain}"
    )
