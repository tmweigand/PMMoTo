...
@pytest.mark.xfail
@pytest.mark.mpi(min_size=8)
def test_too_large_buffer():
    """
    If a buffer spans more than 1 subdomain, an error is raised
    and the use is prompted to change the subdomain topology.
    """
    comm = MPI.COMM_WORLD
    rank = comm.Get_rank()
    ...
    _, _ = pmmoto.core.communication.update_buffer(
        subdomain=sd,
        img=mp.img,
        buffer=buffer,
    )
...
def test_too_large_buffer_solution():
    """
    If a buffer spans more than 1 subdomain, an error is raised
    and the use is prompted to change the subdomain topology.
    """
