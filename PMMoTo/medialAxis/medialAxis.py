...
class medialAxis(object):
    ...
    def genMAArrays(self):
        ...
        """
        dim = self.MA.shape
        tempMA = self.MA[int(self.haloPadNeigh[0]) : int(dim[0] - self.haloPadNeigh[1]),
                         int(self.haloPadNeigh[2]) : int(dim[1] - self.haloPadNeigh[3]),
                         int(self.haloPadNeigh[4]) : int(dim[2] - self.haloPadNeigh[5])]
                
        neighMA = np.pad(self.MA, ( (int(self.haloPadNeighNot[0]), int(self.haloPadNeighNot[1])), 
                                    (int(self.haloPadNeighNot[2]), int(self.haloPadNeighNot[3])), 
                                    (int(self.haloPadNeighNot[4]), int(self.haloPadNeighNot[5])) ), 
