...
def calc_drainage(mp,pc,gamma,nw_phase = 1,w_phase = 2):

    # Get distance from solid to pores
    dist_pm = distance.calc_edt(mp.subdomain,mp.porousmedia.grid)

    #eq_dist = equilibriumDistribution(mp)
    sw = stats.get_saturation(mp.subdomain,mp.grid,phase = w_phase,porousmedia=mp.porousmedia)
    save = True
    ...
    for p in pc:
        if p == 0:
            ...
            r_probe = Multiphase.get_probe_radius(p,gamma)
            ...
            ind = np.where( (dist_pm >= r_probe) & (mp.porousmedia.grid == 1), 1, 0).astype(np.uint8)

            # Step 3 - Check if Points were Marked
            continue_flag = utils.phases_exists(ind,1,mp.index_own_nodes[1])


            if continue_flag:

                # Step 3a and 3d - Check if NW Phases Exists then Collect NW Sets
                nw_check = utils.phases_exists(mp.grid,nw_phase,mp.subdomain.index_own_nodes)
                if nw_check:
 
                    _nw_sets,nw_set_count = sets.collect_sets(mp.grid,nw_phase,mp.inlet[nw_phase],mp.outlet[nw_phase],mp.loop_info[nw_phase],mp.subdomain)
                ...
                w_check = utils.phases_exists(mp.grid,w_phase,mp.subdomain.index_own_nodes)
                if w_check:
 
                    _w_sets = sets.collect_sets(mp.grid,w_phase,mp.inlet[w_phase],mp.outlet[w_phase],mp.loop_info[w_phase],mp.subdomain)
